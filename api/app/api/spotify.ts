...
function validateUrlParam(request: Request): string | null {
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/get-details") {
      const searchQuery = validateUrlParam(request);
      if (!searchQuery)
      ...
      try {
        ...
        return createErrorResponse(
          `Failed to fetch song details: ${error.message}`
        );
      }
    }

    if (url.pathname === "/download") {
      const spotifyUrl = validateUrlParam(request);
      if (!spotifyUrl)
        return createErrorResponse("URL parameter is required", 400);

      try {
        const songDetails = await getSongDetails(spotifyUrl);
        if (songDetails.error) return createErrorResponse(songDetails.message);
        ...
        if (response.error) return createErrorResponse(response.message);
        ...
        return createErrorResponse(`Failed to download song: ${error.message}`);
      }
    }

    return new Response(
      "Spotify Downloader API - Use /get-details?url=... or /download?url=..."
    );
  },
};